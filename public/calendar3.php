...
.default-top, .normal-top { line-height:20px;}
.normal-top { font-size:20px; }
    ...
    }(function ($, moment)
    {
        ...
        $.dateRangePickerLanguages =
        {
            ...
            'fr':
            {
                ...
                'default-less' : 'Indiquer la date de début et la date de fin (ou double-clic si une date)',
        ...
        $.fn.dateRangePicker = function(opt)
        {
            ...
            function showSelectedInfo(forceValid,silent)
            {
                ...
                if (opt.start == opt.end)
                {
                    box.find('.separator-begin').html(opt.singlator);
                    box.find('.start-day').html( getDateString(new Date(parseInt(opt.start))));
                    box.find('.separator-day').html('<i>(Sur un jour et une nuit)</i>');
                    box.find('.end-day').html('');
                } else {
                    box.find('.separator-day').show();
                    if (opt.start)
                    {
                        box.find('.separator-begin').html(opt.beginator);
                        box.find('.start-day').html(getDateString(new Date(parseInt(opt.start))));
                        box.find('.separator-day').html(opt.separator);
                    }
                    if (opt.end)
                    {
                ...
                if (opt.start && opt.singleDate || opt.start == opt.end)
                {
                    ...
                    //var dateRange = opt.singlator +getDateString(new Date(opt.start));
                ...
                else if (opt.start && opt.end)
                {
                    ...
                    box.find('.apply-btn').removeClass('disabled');
                    //var dateRange = opt.beginator +getDateString(new Date(opt.start))+ opt.separator +getDateString(new Date(opt.end));
                    var dateRange = getDateString(new Date(opt.start))+" - "+getDateString(new Date(opt.end));
                    opt.setValue.call(selfDom,dateRange, getDateString(new Date(opt.start)), getDateString(new Date(opt.end)));
                    if (initiated && !silent)
                    {
                        $(self).trigger('datepicker-change',
                ...
                if (opt.showTopbar)
                {
                    ...
                    else
                    {
                        html += '<div class="normal-top">\
                                <span style="color:#333"></span><span class="separator-begin"></span><b class="start-day">...</b>';
                        if ( ! opt.singleDate ) {
...
<body>
    <input type="text" placeholder="JJ/MM/AAAA" class="filterdaterange" value="" 
</body>
</html>
